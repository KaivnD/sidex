/*---------------------------------------------------------------------------------------------
 *  SideX — Cloud/Bun-backed search provider.
...
class CloudSearchProvider extends Disposable implements ISearchResultProvider {
	...
	constructor(
		...
		// Connect to same WebSocket server but for search operations
		const wsUrl = `${serverUrl}?workspaceId=${workspaceId}&type=search`;
	...
	private handleMessage(msg: SearchResponse): void {
		...
		if (!pending) {
			this.logService.warn('[CloudSearch] Unknown request ID:', msg.id);
			return;
		}
		...
		for (const fq of query.folderQueries) {
			...
			try {
				const matches = await this.request<ServerTextMatch[]>('searchText', [
					fq.folder.path, // cloud://workspaceId/path format
				...
				for (const [filePath, fileMatches] of byFile) {
					const resource = URI.parse(filePath);
	...
	async fileSearch(
		...
		for (const fq of query.folderQueries) {
			...
			try {
				...
				for (const m of matches) {
					results.push({ resource: URI.parse(m.path) });
				}
...
export class CloudSearchService extends SearchService {
	...
	constructor(
		...
		this._register(this.registerSearchResultProvider(Schemas.cloud || 'cloud', SearchProviderType.file, this.provider));
		this._register(this.registerSearchResultProvider(Schemas.cloud || 'cloud', SearchProviderType.text, this.provider));
	}
}

registerSingleton(ISearchService, CloudSearchService, InstantiationType.Delayed);
